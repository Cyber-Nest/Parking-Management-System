...
export const PaymentReceiptDrawer = ({ isOpen, onClose, paymentId }: PaymentReceiptDrawerProps) => {
  ...
  const handlePrint = () => {
    ...
    const rows = Object.entries(data)
      .map(
        ([k, v]) =>
          `<tr><td style="padding:6px;border:1px solid #ccc;font-weight:bold;">${k}</td><td style="padding:6px;border:1px solid #ccc;">${String(v)}</td></tr>`,
      )
      .join("");
    w.document.write(
      `<html><head><title>Receipt</title></head><body style="font-family:system-ui;padding:24px;"><h1>Payment receipt</h1><table style="border-collapse:collapse;width:100%;">${rows}</table></body></html>`,
    );
    w.document.close();
    w.focus();
    w.print();
          ...
          >
            <div className="flex items-center justify-between gap-3 mb-4">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-lg font-black text-[var(--color-text-primary)]">Receipt</h2>
              </div>
            ...
            {loading ? (
              ...
              <div className="max-h-[60vh] overflow-y-auto space-y-2 text-sm">
                {Object.entries(data).map(([k, v]) => (
                  <div key={k} className="flex justify-between gap-4 border-b border-[var(--color-border)] py-2">
                    <span className="font-semibold text-[var(--color-text-secondary)]">{k}</span>
                    <span className="text-right text-[var(--color-text-primary)] break-all">{String(v)}</span>
                  </div>
                ))}
              </div>
            ) : (

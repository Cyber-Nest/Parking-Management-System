...
export const AuditLogsSettings = () => {
  ...
  const filteredLogs = useMemo(() => {
    return logs.filter((log) => {
      const userValue = log.user?.toLowerCase() ?? "";
      const actionValue = log.action?.toLowerCase() ?? "";
      const moduleValue = log.module?.toLowerCase() ?? "";
      const searchValue = search.toLowerCase();
      return (
        userValue.includes(searchValue) ||
        actionValue.includes(searchValue) ||
        moduleValue.includes(searchValue)
      );
    });
  }, [logs, search]);
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all border ${showFilters
                  ...
                  >
                    <td className="px-6 py-4">
                      <div className="font-bold text-[var(--color-text-primary)]">
                        {(log.dateTime ?? log.timestamp).split(",")[0]}
                      </div>
                      <div className="text-[11px] text-[var(--color-text-muted)] font-medium">
                        {(log.dateTime ?? log.timestamp).split(",")[1]}
                      </div>
                    </td>
                    <td className="px-6 py-4">
          ...
          <div className="flex items-center gap-1.5">
            ...
            {Array.from({ length: totalPages }).map((_, i) => (
              <button
                ...
                className={`w-9 h-9 flex items-center justify-center rounded-lg text-xs font-bold transition-all ${currentPage === i + 1

...
const StatusBadge = ({ status }: { status: "active" | "inactive" }) => {
  return (
    <span
      className={`px-2.5 py-1 rounded-full text-[10px] font-bold uppercase ${status === "active"
...
export const UsersRolesSettings = () => {
  ...
  const handleToggleUserStatus = async (user: User) => {
    try {
      ...
      setUsers((prev) =>
        prev.map((u) =>
          u.id === user.id
            ? { ...u, status: response.user?.status ?? user.status }
            : u,
        ),
      );
    } catch (error) {
  ...
  const handlePermissionChange = (
    ...
    setRoleForm((prev) => ({
      ...
      permissions: {
        ...prev.permissions,
        [moduleId as PermissionModule]: {
          ...prev.permissions[moduleId as PermissionModule],
          [actionId as PermissionAction]: value,
        },
      },
    }));
  };
      ...
      <div className="bg-[var(--color-surface)] p-8 md:p-10 rounded-[28px] border border-[var(--color-border)] shadow-sm">
        ...
        <div className="flex gap-2 mb-6">
          ...
          <button
            ...
            className={`px-6 py-2.5 rounded-xl text-sm font-bold transition-all ${activeTab === "roles"
              ? "bg-[var(--color-primary)] text-white shadow-md"
              : "bg-[var(--color-bg)] text-[var(--color-text-secondary)] hover:bg-[var(--color-surface-soft)]"
              }`}
          >
              ...
              <div className="space-y-2">
                {roles.map((role) => (
                  <button
                    ...
                    className={`w-full p-4 rounded-xl text-left transition-all ${selectedRole?.id === role.id

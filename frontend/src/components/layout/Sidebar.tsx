    ...
    <>
      ...
      <aside
        className={`
          ...
          text-white/90 dark:text-[var(--color-text-secondary)]
          transform ${isOpen ? "translate-x-0" : "-translate-x-full"}
          md:translate-x-0 transition-transform duration-300 ease-[cubic-bezier(0.23,1,0.32,1)]
            ...
            if (item.isDropdown) {
              return (
                <div key={item.label} className="space-y-1">
                  <button
                    onClick={() => setIsReportsOpen(!isReportsOpen)}
                    className="w-full flex items-center justify-between px-4 py-2.5 md:py-3 rounded-xl text-white/60 dark:text-[var(--color-text-secondary)] hover:text-white dark:hover:text-[var(--color-text-primary)] hover:bg-white/5 dark:hover:bg-[var(--color-primary)]/10 transition-all group"
                  >
                    <div className="flex items-center gap-3">
                      <item.icon size={18} className="md:w-5 md:h-5" />
                  ...
                  <AnimatePresence>
                    {isReportsOpen && (
                      <motion.div
                        ...
                        animate={{ height: "auto", opacity: 1 }}
                        exit={{ height: 0, opacity: 0 }}
                        className="relative ml-6 border-l border-white/10 dark:border-[var(--color-border)] overflow-hidden flex flex-col"
                      >
                        {item.subItems?.map((sub) => {
                          const isSubActive = pathname === sub.href;
                          return (
            ...
            return (
              ...
              >
                ...
                >
                  ...
                  <item.icon
                    size={18}
                    className={`relative z-10 md:w-5 md:h-5 ${isActive ? "text-[var(--color-primary)] dark:text-white" : "group-hover:scale-110 transition-transform"}`}
                  />
                  <span className="relative z-10 font-medium text-[13px] md:text-[14px]">
                    {item.label}
                  </span>

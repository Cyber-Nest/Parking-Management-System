...
export default function OfficerTicketsPage() {
  ...
  const [selectedTicket, setSelectedTicket] = useState<OfficerTicket | null>(
    null,
  );
  ...
  const [modalType, setModalType] = useState<
    "payment" | "evidence" | "review" | "view" | "edit" | "delete" | null
  >(null);
  ...
  useEffect(() => {
    ...
    if (cached) {
      try {
        const parsed = JSON.parse(cached) as OfficerProfile;
    ...
    void officerPortalService
      ...
      .then((profile) => {
        ...
        window.localStorage.setItem("officerProfile", JSON.stringify(profile));
      })
      .catch(() => {
  ...
  const filteredTickets = useMemo(() => {
    const active = tickets.filter((ticket) => {
      if (activeTab === "my") {
        return officerProfile ? ticket.officer_id === officerProfile.id : false;
      }
      if (activeTab !== "all") {
    ...
    const withLotFilter = lotFilter
      ? active.filter((ticket) =>
          ticket.location_name
            ?.toLowerCase()
            .includes(lotSelected?.lot_name.toLowerCase() ?? ""),
        )
      : active;
    ...
    try {
      ...
      if (type === "edit") {
        setEditForm({
          ...
          dueDate: freshTicket.due_date
            ? new Date(freshTicket.due_date).toISOString().slice(0, 10)
            : "",
        });
      }
    } catch {
      if (type === "edit") {
        setEditForm({
          ...
          dueDate: ticket.due_date
            ? new Date(ticket.due_date).toISOString().slice(0, 10)
            : "",
        });
      }
    }
  ...
  const submitEvidence = async () => {
    ...
    try {
      ...
      setEvidenceError(
        getApiErrorMessage(error, "Unable to add evidence. Please try again."),
      );
    } finally {
      setIsSaving(false);
    }
  ...
  const submitPayment = async () => {
    ...
    try {
      ...
      toast.error(
        getApiErrorMessage(
          error,
          "Unable to record payment. Please check the details and try again.",
        ),
      );
    } finally {
      setIsSaving(false);
  ...
  const submitReview = async () => {
    ...
    try {
      setIsSaving(true);
      setReviewError(null);
      await officerEnforcementService.reviewTicket(
        selectedTicket.id,
        reviewNote.trim(),
      );
      await reloadTickets();
      ...
      setReviewError(
        getApiErrorMessage(error, "Unable to send ticket to review."),
      );
    } finally {
      setIsSaving(false);
    }
  ...
  const submitEdit = async () => {
    ...
    if (
      !Number.isFinite(Number(editForm.fineAmount)) ||
      Number(editForm.fineAmount) <= 0
    ) {
    ...
    try {
      setIsSaving(true);
      const updated = await officerEnforcementService.updateTicket(
        selectedTicket.id,
        {
      ...
      <div id="ticket-print-section" className="hidden">
        {selectedTicket ? (
          <div className="ticket-print-header">
            ...
            <p className="text-sm">
              Location: {selectedTicket.location_name ?? "Unknown"}
            </p>
            <p className="text-sm">Officer: {selectedTicket.officer_name}</p>
            <p className="text-sm">
              Issued: {new Date(selectedTicket.date_issued).toLocaleString()}
            </p>
          </div>
        ) : null}
        {selectedTicket ? (
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p className="text-sm text-slate-500">
              View, filter and manage issued enforcement tickets.
            </p>
          </div>
          <div className="flex flex-wrap gap-3">
            ...
            <button
              ...
              className="inline-flex items-center gap-2 rounded-md bg-[#1062ff] px-4 py-2 text-sm font-semibold text-white disabled:opacity-50"
            >
            ...
            <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
              <div className="rounded-3xl border border-slate-200 bg-white p-5">
                ...
                <p className="mt-2 text-sm text-slate-500">
                  Tickets still awaiting payment
                </p>
              </div>
              <div className="rounded-3xl border border-slate-200 bg-white p-5">
                <p className="text-xs uppercase tracking-[0.24em] text-slate-500">
              ...
              <div className="grid gap-3 lg:grid-cols-5">
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="No Parking, Expired Meter"
                    className="mt-2 w-full rounded-2xl border border-slate-200 bg-white px-3 py-2 text-sm outline-none"
                  />
                </div>
                ...
                filteredTickets.map((ticket) => (
                  <div
                    ...
                    className={`rounded-3xl border bg-white p-4 shadow-sm transition ${
                      selectedTicket?.id === ticket.id
                        ? "border-[#1062ff] ring-1 ring-[#1062ff]/20"
                  ...
                  >
                    <div className="flex items-start justify-between gap-4">
                      ...
                      >
                        ...
                        <p className="mt-2 text-xs text-slate-400">
                          {new Date(ticket.date_issued).toLocaleString()}
                        </p>
                      </div>
                        ...
                        {openActionId === ticket.id ? (
                          <div className="absolute right-0 top-12 z-20 w-40 overflow-hidden rounded-xl border border-slate-200 bg-white py-1 text-left shadow-xl">
                            <button
                              type="button"
                              onClick={() =>
                                void openTicketModal(ticket, "view")
                              }
                            ...
                            <button
                              type="button"
                              onClick={() =>
                                void openTicketModal(ticket, "edit")
                              }
                              className="flex w-full items-center gap-2 px-3 py-2 text-sm text-slate-700 hover:bg-slate-50"
                            >
                              <Edit3 size={14} /> Edit
                            ...
                            <button
                              type="button"
                              onClick={() =>
                                void openTicketModal(ticket, "delete")
                              }
                    ...
                    <div className="mt-4 flex flex-wrap gap-2 text-sm text-slate-600">
                      ...
                      <span
                        className={`rounded-full px-2 py-1 text-xs font-semibold ${ticket.status === "paid" ? "bg-emerald-50 text-emerald-700" : ticket.status === "disputed" ? "bg-amber-50 text-amber-700" : ticket.status === "cancelled" ? "bg-slate-100 text-slate-700" : "bg-rose-50 text-rose-700"}`}
                      >
                        {ticket.status}
                      </span>
                    </div>
                  ...
                  {isLoading ? (
                    ...
                    filteredTickets.map((ticket) => (
                      ...
                      >
                        ...
                        <td className="px-5 py-4">
                          <span
                            className={`rounded-full px-3 py-1 text-xs font-bold ${ticket.status === "paid" ? "bg-emerald-50 text-emerald-700" : ticket.status === "disputed" ? "bg-amber-50 text-amber-700" : ticket.status === "cancelled" ? "bg-slate-100 text-slate-700" : "bg-rose-50 text-rose-700"}`}
                          >
                            {ticket.status}
                          </span>
                        </td>
              ...
              <div className="mb-4 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-slate-500">
                    Selected record summary and actions.
                  </p>
                </div>
                <button
                ...
                <div className="space-y-4">
                  <div className="rounded-3xl border border-slate-100 bg-slate-50 p-4">
                    ...
                    <p className="mt-3 text-sm text-slate-500">
                      Issued on{" "}
                      {new Date(selectedTicket.date_issued).toLocaleString()}
                    </p>
                  ...
                  <div className="space-y-3 rounded-3xl border border-slate-100 bg-slate-50 p-4 text-sm text-slate-700">
                    <DetailRow
                      label="License Plate"
                      value={selectedTicket.license_plate}
                    />
                    <DetailRow
                      label="Violation"
                    ...
                    <DetailRow
                      label="Location"
                      value={selectedTicket.location_name ?? "Unknown"}
                    />
                    <DetailRow
                      label="Officer"
                      value={selectedTicket.officer_name}
                    />
                    <DetailRow
                      label="Amount"
                      value={`$${Number(selectedTicket.amount).toFixed(2)}`}
                    />
                    <DetailRow
                      label="Due Date"
                      value={
                    ...
                    />
                    {selectedTicket.note ? (
                      <DetailRow label="Remarks" value={selectedTicket.note} />
                    ) : null}
                  </div>
                  <div className="grid gap-3">
                    <button
                      onClick={printTicket}
                      disabled={!selectedTicket}
                      className="inline-flex w-full items-center justify-center gap-2 rounded-2xl bg-[#1062ff] py-3 text-sm font-semibold text-white disabled:opacity-50"
                    >
                      <Printer size={16} /> Print Ticket
                    </button>
                    <button
                      ...
                      className="inline-flex w-full items-center justify-center gap-2 rounded-2xl border border-slate-200 bg-white py-3 text-sm font-semibold text-slate-700 disabled:opacity-50"
                    >
            ...
            <div className="w-full max-w-lg rounded-3xl border border-slate-200 bg-white p-6 shadow-2xl">
                      ...
                      : modalType === "edit"
                        ...
                        : modalType === "delete"
                          ...
                          : modalType === "payment"
                            ...
                            : modalType === "evidence"
                              ? "Add Evidence"
                              : "Send Ticket to Review"}
                  </p>
                  <p className="mt-1 text-sm text-slate-500">
                    {modalType === "view"
                      ? "Review the full ticket record."
                      : modalType === "edit"
                        ? "Update ticket fields and save changes."
                        : modalType === "delete"
                          ? "Confirm this ticket should be removed from active work."
                          : modalType === "payment"
                            ? "Enter payment details for this ticket."
                            : modalType === "evidence"
                              ? "Attach evidence to the selected ticket."
                              : "Add a review note for this ticket."}
                  </p>
                </div>
                <button
              ...
              {modalType === "view" && selectedTicket ? (
                <div className="mt-6 max-h-[70vh] space-y-4 overflow-y-auto pr-2">
                  ...
                  />
                  {selectedTicket.note ? (
                    <DetailRow label="Notes" value={selectedTicket.note} />
                  ) : null}
                  ...
                  <div className="flex justify-end gap-3 pt-4">
                    ...
                    <button
                      onClick={() =>
                        void openTicketModal(selectedTicket, "edit")
                      }
                      className="rounded-2xl bg-[#1062ff] px-5 py-3 text-sm font-semibold text-white"
                    >
                      Edit
                    </button>
                  </div>
                ...
                <div className="mt-6 space-y-4">
                  <p className="rounded-2xl border border-rose-200 bg-rose-50 px-4 py-3 text-sm text-rose-700">
                    Delete ticket {selectedTicket.ticket_number}? This will mark
                    the ticket as cancelled so it no longer behaves like an
                    active violation.
                  </p>
                  <div className="flex justify-end gap-3 pt-4">
                    <button
                ...
                <div className="mt-6 space-y-4">
                  <input
                    ...
                    accept="image/*"
                    className="hidden"
                    onChange={(event) => void handleEvidenceFilePick(event)}
                  />
                  <label className="block text-xs font-semibold text-slate-500">
                    Evidence Photo
                  </label>
                ...
                <div className="mt-6 space-y-4">
                  ...
                  <div className="flex justify-end gap-3 pt-4">
                    ...
                    <button
                      ...
                      disabled={
                        isSaving ||
                        Boolean(getReviewBlockMessage(selectedTicket))
                      }
                      className="rounded-2xl bg-[#1062ff] px-5 py-3 text-sm font-semibold text-white disabled:opacity-50"
                    >
                      Send Review

...
export default function PaymentReconciliationReport() {
  ...
  const [tableData, setTableData] = useState<ReconciliationData[]>([]);
  ...
  const filteredData = useMemo(() => {
    ...
    return tableData.filter((row) => row.date.toLowerCase().includes(query));
  ...
  const fetchAllData = useCallback(async () => {
    try {
      setLoading(true);
      const [statsRes, tableRes, chartRes, paymentRes] = await Promise.all([
        paymentReconciliationService.getStats(filters),
        paymentReconciliationService.getTableData(filters),
        paymentReconciliationService.getChartData(filters),
        paymentReconciliationService.getPaymentMethodData(filters),
      ]);
      setStats(statsRes);
      setTableData(tableRes);
      setChartData(chartRes);
      setPaymentMethodData(paymentRes);
      setCurrentPage(1);
    } catch (error) {
      console.error(error);
  ...
  const handleExport = async () => {
    try {
      setExporting(true);
      const response = await paymentReconciliationService.exportReport(filters);
  ...
  // Handle view details
  const handleViewDetails = (item: ReconciliationData) => {
  ...
  const paymentMethodOptions = ["All Methods", "Card", "Cash", "Other"];
  const statusOptions = ["All Status", "Reconciled", "Pending", "Failed"];
  ...
  return (
    <div className="min-h-screen px-4 md:px-6 py-6 space-y-8 bg-[var(--color-bg)]">
      ...
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-[var(--color-text-secondary)] font-semibold mt-1">
            Reconcile collected payments with deposits and refunds.
          </p>
        </div>
        ...
        stats && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <StatCard
              ...
              title="Deposited"
              value={`$${stats.deposited.toLocaleString()}`}
            />
            <StatCard
            ...
            <div className="bg-[var(--color-surface)] p-5 rounded-2xl border border-[var(--color-border)] shadow-sm">
              ...
              <div className="flex justify-end gap-3 mt-4">
                ...
                {/* <button
                  ...
                  className="flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-bold bg-[var(--color-primary)] text-white hover:bg-[var(--color-primary-dark)] transition-all"
                >
                  Apply Filters
                </button> */}
              </div>
      ...
      {/* Charts Section */}
      <ReconciliationCharts chartData={chartData} loading={loading} />
          ...
          <table className="w-full text-left border-collapse min-w-[1000px]">
            <thead className="bg-[var(--color-surface-soft)] border-b border-[var(--color-border)]">
              <tr className="text-[10px] uppercase text-[var(--color-text-muted)] font-black tracking-widest">
                ...
                <th className="px-4 sm:px-6 py-4">Collected (Total)</th>
                <th className="px-4 sm:px-6 py-4">Card Amount</th>
                <th className="px-4 sm:px-6 py-4">Refunds</th>
                <th className="px-4 sm:px-6 py-4">Deposited</th>
                <th className="px-4 sm:px-6 py-4">Net Expected</th>
                <th className="px-4 sm:px-6 py-4 text-center">Variance</th>
              ...
              {loading ? (
                ...
                paginatedData.map((row, idx) => (
                  ...
                  >
                    ...
                    <td className="px-4 sm:px-6 py-4 font-bold text-[var(--color-text-primary)]">
                      ${row.netExpected.toLocaleString()}
                    </td>
                    <td className="px-4 sm:px-6 py-4 text-center">
                      <span
                        className={`font-bold ${row.variance === 0 ? "text-emerald-500" : "text-rose-500"}`}
                      >

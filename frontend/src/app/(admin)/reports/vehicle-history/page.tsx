...
export default function VehicleHistoryReport() {
  ...
  const handleExport = async (format: "pdf" | "excel") => {
    try {
      setExporting(true);
      setShowExportDropdown(false);
      const response = await vehicleHistoryService.exportReport({
        ...filters,
        licensePlate: searchedPlate,
        tab: activeTab,
        format: format,
      ...
      if (response.blob) {
        ...
        link.href = url;
        link.setAttribute("download", `vehicle_history_${searchedPlate}_${new Date().toISOString().split('T')[0]}.${format === "pdf" ? "pdf" : "xlsx"}`);
        ...
        <div className="p-5">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            {filters.dateRange === "Custom Range" && (
              <>
                <div className="space-y-1.5">
                  <label className="text-[10px] font-black uppercase text-[var(--color-text-muted)] tracking-wider">
                    Start Date
                  </label>
                  <input
                    type="date"

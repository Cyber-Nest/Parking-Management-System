...
export const penaltyReportService = {
  ...
  async getTicketsData(
    ...
    return items.map((t: any) => ({
      date: t.date_issued
        ? new Date(t.date_issued).toISOString().slice(0, 10)
        : "",
  ...
  async getPenaltyTrend(
    filters: PenaltyReportFilters & { period: string },
  ...
  async exportReport(
    ...
    return downloadReportExport("penalty", format, {
      from,
      to,
      parking_lot_id,
    });

...
export const locationPerformanceService = {
    ...
    async getRevenueByLocation(filters: LocationPerformanceFilters): Promise<LocationChartDatum[]> {
        const rows = await locationPerformanceService.getLocationsData(filters);
        const colors = ["#6366f1", "#22c55e", "#f97316", "#ec4899"];
        return rows.map((r, i) => ({ name: r.location, value: r.revenue, rate: r.revenue, color: colors[i % colors.length] }));
    },

    async getOccupancyData(filters: LocationPerformanceFilters): Promise<LocationChartDatum[]> {
        const rows = await locationPerformanceService.getLocationsData(filters);
        return rows.map((r, i) => ({
            name: r.location,
    ...
    async exportReport(payload: LocationPerformanceFilters & { format: ReportExportFormat }) {
    ...
    async getLocationDetails(_locationId: string): Promise<LocationDetails> {
        return {
            name: "Location",
            address: "—",
    ...
    async getLocationMetrics(_locationId: string): Promise<LocationPerformanceMetrics> {
        void _locationId;
        return {
            dailyRevenue: 0,
            totalSessions: 0,
    ...
    async getLocationRevenueTrend(_locationId: string): Promise<LocationRevenueTrend[]> {
        void _locationId;
    ...
    async getLocationSessionTrend(_locationId: string): Promise<LocationSessionTrend[]> {
        void _locationId;
        return Array.from({ length: 7 }, (_, i) => ({
            date: `Day ${i + 1}`,
    ...
    async getLocationOfficers(_locationId: string): Promise<LocationOfficerData[]> {
        void _locationId;
        return [];
    },
    async getLocationVehicleHistory(_locationId: string): Promise<LocationVehicleHistory[]> {
        void _locationId;
        return [];
    },

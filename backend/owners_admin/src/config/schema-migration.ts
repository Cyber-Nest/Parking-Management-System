...
export const ensureParkingPlanSchema = async (): Promise<void> => {
  ...
  try {
    const [lotRows] = await conn.query<(RowDataPacket & { COLUMN_NAME: string })[]>(
      `SELECT COLUMN_NAME
    ...
    const [zoneRows] = await conn.query<(RowDataPacket & { COLUMN_NAME: string })[]>(
      `SELECT COLUMN_NAME
       FROM INFORMATION_SCHEMA.COLUMNS
       WHERE TABLE_SCHEMA = DATABASE()
         AND TABLE_NAME = 'parking_plans'
    ...
    if (zoneRows.length === 0) {
      await conn.query(
        `ALTER TABLE parking_plans ADD COLUMN parking_zone_id VARCHAR(60) NULL AFTER parking_lot_id`,
      );
      console.log("[DB] Added missing parking_plans.parking_zone_id column");
    } else {
      console.log("[DB] parking_plans.parking_zone_id already exists");
    }
  } finally {
    conn.release();
  }

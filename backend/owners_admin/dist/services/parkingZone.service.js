...
async function resolveZoneImageUrl(raw) {
    const value = String(raw ?? '').trim();
    if (!value)
        return parkingImages_1.DEFAULT_PARKING_IMAGE;
    if ((0, cloudinary_service_1.isCloudinaryUrl)(value))
        return value;
    if (/^data:image\//i.test(value)) {
        return (0, cloudinary_service_1.ensureCloudinaryUrl)(value, { folder: 'parksmart/zones' });
    }
    return (0, parkingImages_1.sanitizeParkingImageUrl)(value);
}
class ParkingZoneService {
    ...
    async create(body) {
        ...
        const image_url = await resolveZoneImageUrl(body.image_url ?? body.imageUrl ?? parkingImages_1.DEFAULT_PARKING_IMAGE);
